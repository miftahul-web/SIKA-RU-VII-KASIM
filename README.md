...
<body>
  <header>
    <div style="width:28px;height:28px;display:flex;align-items:center;justify-content:center;background:#fff;border-radius:6px">🛰️</div>
    <div class="title">Monitoring Area Kerja — Citra Satelit (Izin Kerja)</div>
    <button id="toggleSave" title="Simpan/Restore" class="btn" style="background:#fff;border-radius:8px">💾</button>
  </header>
  ...
  <div id="app">
    <div class="sidebar" id="sidebar">
      ...
      <label>Radius (meter) — untuk Lingkaran</label>
      ...
      <div class="info footer">
        • Klik peta untuk mengisi koordinat.<br>
        • Gambar poligon gunakan tombol draw di pojok kiri peta.<br>
        • Data tersimpan otomatis ke browser (localStorage). Gunakan Export/Import untuk backup.<br>
        • Tanggal kadaluarsa izin dihitung berakhir pada jam 23:59 pada tanggal yang dipilih (izin tetap berlaku sepanjang hari tersebut).
      </div>
    </div>
  ...
  <script>
    ...
    function rebuildList(){
      ...
      areas.forEach(a => {
        ...
        const regDateText = a.regDate ? ` • Reg: ${a.regDate}` : '';
        ...
        const hiddenNote = a.hiddenByExpiry ? ' • (dihilangkan dari peta: kadaluarsa)' : '';
        meta.innerHTML = `<div><strong>${a.reg||'(no)'} - ${a.name||'(tanpa nama)'}</strong>
                          <div class="small">Perusahaan: ${a.company||'-'} • Izin: ${a.permit||'-'} • ${a.type} • ${a.crit} • ${statusText}${hiddenNote}${regDateText}</div></div>`;
        ...
        if(a.type==='point' || a.type==='circle'){
          ...
          info.textContent = `Perusahaan: ${a.company || '-'}  •  Tanggal Registrasi: ${a.regDate || '-'}  •  Lokasi: ${a.location || '-'}  •  Status Lokasi: ${a.locationStatus || '-'}  •  Jenis Izin: ${a.workType || '-'}  •  Status Izin: ${a.statusPermit || '-'}  •  Tenggat Izin: ${a.expiry || '-'}`;
          div.appendChild(p);
    ...
    function addAreaToMap(obj, fit=true){
      ...
      if(obj.type==='polygon' && obj.geojson){
        ...
        layer.eachLayer(l => l.bindPopup(`<b>${obj.reg||''} - ${obj.name||''}</b><br>Perusahaan: ${obj.company||'-'}<br>Izin: ${obj.permit||''}<br>${obj.crit}<br>Jenis Izin: ${workType||'-'}<br>Status Izin: ${statusPermit||'-'}<br>Expiry: ${obj.expiry||'-'}<br>Reg: ${obj.regDate||'-'}<br>Lokasi: ${loc||'-'} • ${locStatus}`));
      } else if(obj.type==='circle'){
        layer = L.circle([obj.lat,obj.lng], { radius: obj.radius||50, color: colorForCrit(obj.crit), fillOpacity: 0.18 }).addTo(map);
        layer.bindPopup(`<b>${obj.reg||''} - ${obj.name||''}</b><br>Perusahaan: ${obj.company||'-'}<br>Izin: ${obj.permit||''}<br>${obj.crit}<br>Jenis Izin: ${workType||'-'}<br>Status Izin: ${statusPermit||'-'}<br>Expiry: ${obj.expiry||'-'}<br>Reg: ${obj.regDate||'-'}<br>Lokasi: ${loc||'-'} • ${locStatus}`);
      } else { // point
    ...
    map.on(L.Draw.Event.CREATED, function (event) {
      ...
      let regDate = prompt('Tanggal Registrasi (YYYY-MM-DD) — kosongkan jika tidak ada:','') || '';
      ...
      let expiry = prompt('Tanggal kadaluarsa izin (YYYY-MM-DD) — kosongkan jika tidak ada:','') || '';
      ...
      layer.bindPopup(`<b>${reg} - ${name}</b><br>Perusahaan: ${company||'-'}<br>Izin: ${permit}<br>${crit}<br>Jenis Izin: ${workType||'-'}<br>Status Izin: ${statusPermit||'-'}<br>Expiry: ${expiry||'-'}<br>Reg: ${regDate||'-'}<br>Lokasi: ${location||'-'} • ${locationStatus}`);
  ...
  <div id="floatingDuePanel" aria-live="polite" role="region">
    <div id="floatingDueHeader">
      ...
      <div class="controls">
        <button id="refreshDue" class="small-btn" title="Refresh">🔄</button>
        <button id="minimizeDue" class="small-btn" title="Minimize/Restore">—</button>
        <button id="closeDue" class="small-btn" title="Tutup">✕</button>
      </div>
    </div>
  ...
  (function(){
    ...
    function buildList(){
      ...
      for(const it of dueItems){
        ...
        const title = document.createElement('div'); title.className='due-title'; title.textContent = `${a.reg || '(no)'} — ${a.name || '(tanpa nama)'}`;
        const sub = document.createElement('div'); sub.className='due-sub';
        const dueStr = it.due.toLocaleString();
        sub.innerHTML = `Perusahaan: ${a.company || '-'} • Izin: ${a.permit || '-'} • Lokasi: ${a.location || '-'} • Status Lokasi: ${a.locationStatus || '-'} • Jenis Izin: ${a.workType || '-'} • Status Izin: ${a.statusPermit || '-'} • ${a.type || '-'} • ${a.crit || '-' }<br><span style="font-weight:600">Tenggat: ${dueStr}</span>`;
